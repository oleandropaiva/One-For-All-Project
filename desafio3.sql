...
ROUND(SUM(cancoess.duracao_segundos /60), 2) AS total_minutos
	...
	INNER JOIN 
        SpotifyClone.reproducoes_table AS reproducoess ON usuarioss.usuario_id = reproducoess.usuario_id
	INNER JOIN     
        SpotifyClone.cancoes_table AS cancoess ON cancoess.cancao_id = reproducoess.historico_de_reproducoes
	GROUP BY usuario;
